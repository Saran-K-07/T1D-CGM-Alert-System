...
public final class SecureAlertMessageCodec {
    ...
    public static String encrypt(PreferencesRepository prefs, String plainMessage) {
        String sharedKey = AlertKeyManager.getOrCreateSharedAlertKey(prefs);
        return encryptWithSharedKey(sharedKey, plainMessage);
    }
    ...
    public static DecodeResult decrypt(PreferencesRepository prefs, String message) {
        if (message == null || message.trim().isEmpty()) {
            return DecodeResult.invalid("empty_message");
        }
        ...
        String sharedKey = prefs.getString(AppPrefs.KEY_SHARED_ALERT_KEY, "").trim();
        return decryptWithSharedKey(sharedKey, trimmed);
    }
